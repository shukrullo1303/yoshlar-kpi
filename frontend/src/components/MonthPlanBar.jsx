...
// defaultWeekly: default number of plans per week (null = no default pre-fill)
const PLAN_CONFIG = {
...
function getMonthWeeks(monthStr) {
  ...
  while (start <= daysInMonth) {
    ...
    weeks.push({
      startDate: `${year}-${pad(mon + 1)}-${pad(start)}`,
      label: `${start}–${end} ${UZ_MONTHS[mon]}`,
      startDay: start,
      endDay: end,
    })
    start += 7
...
// Map loaded plan_dates entries (per-day or per-week) into weekCounts
...
export function MonthPlanBar({ direction, month, maxScore, defaultOpen = false }) {
  const [weekCounts, setWeekCounts] = useState({})  // { startDate: count }
    ...
    api.getMonthPlan(direction, monthStr)
      .then(data => {
        ...
        if (norm.length > 0) {
          ...
          // Pre-fill with direction's default weekly count
          const defaults = {}
          for (const week of weeks) defaults[week.startDate] = config.defaultWeekly
      ...
      {open && (
        <div className="border-t border-slate-700 p-5 space-y-3">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide">
            Haftalar bo'yicha reja{' '}
            <span className="font-normal normal-case text-slate-600">
              — hafta yoniga son kiriting yoki belgilang
            </span>
          </p>
                ...
                <div
                  ...
                  className={`flex items-center gap-3 rounded-xl px-4 py-3 transition-all border ${
                    isSelected
                      ...
                      : 'bg-slate-700/40 border-transparent'
                  }`}
                >
                  {/* Checkbox */}
                  <button
                  ...
                  {/* Week label */}
                  <div className="flex-1 min-w-0">
                    <span className="text-[11px] font-bold text-slate-500">{i + 1}-hafta</span>
                    <p className="text-sm font-medium text-slate-200 leading-tight">{week.label}</p>
                  </div>
                  ...
                  <input
                    ...
                    className={`w-20 text-center rounded-lg px-2 py-1.5 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all ${
                      isSelected
          ...
          {/* Running total */}
          {totalCount > 0 && (
            <div className="flex items-center justify-between px-4 py-2.5 bg-slate-700/40 rounded-lg">
